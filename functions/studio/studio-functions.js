...
function completeReversal(arr) {
    for (item in arr) {
        newArr.push(reverseCharacters(arr[item]));
    }
    return newArr;
}
...
function funPhrase(str) {
    return `We put the ${(str.length <= 3) ? str[str.length - 1] : str.slice(0, 3)} in ${str}.`;
}
...
console.log(`The area is ${rectangle(5)} cm^2`);

console.log(`The area is ${rectangle(5, 10)} cm^2`);